module fishnft::FishNFT {
    ...
    const OWNER_ADDR:address = @0x66f1b06b4b3f563bb54252937f7ca2066260001253e1a7d0b58081dd045b6f3b;
...
//    }
    ...
    public entry fun burn(nft: FishNFT,ctx: &mut TxContext) {
        let msg_sender = tx_context::sender(ctx);
        assert!(nft.owner == msg_sender , 0);
        let FishNFT { id, name: _, owner:_, url: _, token_type: _,weigtht:_ } = nft;
        object::delete(id);
    }
...
module fishnft::FishNFTTest{
    ...
    fun mint_transfer(){
        ...
        {   
            FishNFT::mint(b"Shite",1223,ts::ctx(&mut scenario));
        };
        ...
        {
            FishNFT::mint(b"QFQEFEf",2352333,ts::ctx(&mut scenario2))
        };
